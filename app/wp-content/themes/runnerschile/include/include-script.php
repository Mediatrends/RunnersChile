<?php
	...
	*/ 
	...
	function register_non_admin_styles(){
		...
		// Page post_type
			...
			if(	strpos($gdl_page_xml,'<slider-type>Flex Slider</slider-type>') > -1 || 
				...
				$gdl_top_slider_type == 'Flex Slider' || $gdl_top_slider_type == 'Post Slider'){
				wp_enqueue_style('flex-slider',GOODLAYERS_PATH.'/stylesheet/flexslider.css');
			}
	...
	function register_non_admin_scripts(){
		...
		if( $gdl_top_sliding ){
			wp_deregister_script('marquee');
			wp_register_script('marquee', GOODLAYERS_PATH.'/javascript/marquee.js', false, '1.0', true);
