<?php
	...
	*/
	...
	function gdl_get_style_custom_content(){
		...
		$temp_att = gdl_style_att( 'padding-top', get_option(THEME_SHORT_NAME . "_logo_top_margin", '42') . 'px' );
		...
		$temp_val = get_option(THEME_SHORT_NAME . "_banner_max_width", '');
		if( !empty($temp_val) ){
			$temp_att = gdl_style_att( 'max-width', $temp_val . 'px' );
			gdl_print_style( '.header-right-banner-wrapper', $temp_att );		
		}		
		$temp_att = gdl_style_att( 'margin-top', get_option(THEME_SHORT_NAME . "_banner_top_margin", '35') . 'px' );
		gdl_print_style( '.header-right-banner-wrapper', $temp_att );		
		...
		$temp_att = gdl_style_att( 'font-size', get_option(THEME_SHORT_NAME . "_widget_title_size", '18') . 'px' );
		gdl_print_style( 'h3.custom-sidebar-title, h3.gdl-header-title', $temp_att );			
		...
		gdl_print_style( 'h1, h2, h3, h4, h5, h6, div.page-header-caption, div.gdl-social-counter-widget span.count, ' . 
			'div.price-item .price-title, div.price-item .price-tag, .portfolio-item-filter ', $temp_att );			
		...
		gdl_print_style( 'div.main-navigation-wrapper', $temp_att );			
		
		// Top Search Icon Type
		$gdl_icon_type = get_option(THEME_SHORT_NAME.'_top_search_icon_type','light');

			$temp_att = gdl_style_att( 'background', 'url(' . GOODLAYERS_PATH . '/images/icon/' . $gdl_icon_type . '/top-search.png) no-repeat right center;' );
			...
			gdl_write_data( 'only screen and (-webkit-min-device-pixel-ratio: 2), only screen and (min-device-pixel-ratio: 2) {' );
				$temp_att = gdl_style_att( 'background', 'url(' . GOODLAYERS_PATH . '/images/icon/' . $gdl_icon_type . '/top-search@2x.png) no-repeat left center;' );
				gdl_print_style( "div.top-search-wrapper input[type='submit']", $temp_att );
			gdl_write_data( '}' );	

		// Post Sliding
		$gdl_icon_type = get_option(THEME_SHORT_NAME.'_post_sliding_icon_type','light');
		
			$temp_att = gdl_style_att( 'background', 'url(' . GOODLAYERS_PATH . '/images/icon/' . $gdl_icon_type . '/personnal-widget-right.png) no-repeat 0px center;' );
			gdl_print_style( "div.header-top-marquee .marquee-wrapper .marquee div", $temp_att );	
			$temp_att = gdl_style_att( 'background-image', 'url(' . GOODLAYERS_PATH . '/images/icon/' . $gdl_icon_type . '/breaking-news-head.png);' );
			gdl_print_style( "div.header-top-marquee .marquee-head", $temp_att );	
			
		// Icon Type
		$gdl_icon_type = get_option(THEME_SHORT_NAME.'_icon_type','dark');
			
			if( $gdl_icon_type == 'dark' ){
				gdl_write_data( '.blog-info-wrapper i, div.gdl-blog-list .blog-comment a{ color: #3b3b3b; }' );
			}else{
				gdl_write_data( '.blog-info-wrapper i, div.gdl-blog-list .blog-comment a{ color: #ffffff; }' ); 
			}
			...
			gdl_print_style( 'ul.gdl-accordion li, ul.gdl-toggle-box li, div.gdl-carousel-testimonial .testimonial-content', $temp_att );				
			...
			gdl_print_style( 'div.blog-carousel-wrapper .blog-nav.left, div.portfolio-carousel-wrapper .port-nav.left, div.portfolio-carousel-description .port-nav.left, .testimonial-navigation .testimonial-prev, div.single-portfolio .port-prev-nav a', $temp_att );				
			$temp_att = gdl_style_att( 'background-image', 'url(' . GOODLAYERS_PATH . '/images/icon/' . $gdl_icon_type . '/nav-right.png)' );
			gdl_print_style( 'div.blog-carousel-wrapper .blog-nav.right, div.portfolio-carousel-wrapper .port-nav.right, div.portfolio-carousel-description .port-nav.right, .testimonial-navigation .testimonial-next, div.single-portfolio .port-next-nav a', $temp_att );	
		...
		$gdl_twitter_icon_type = get_option(THEME_SHORT_NAME.'_twitter_icon_type','light');
			...
			$temp_att = gdl_style_att( 'background-image', 'url(' . GOODLAYERS_PATH . '/images/icon/' . $gdl_twitter_icon_type . '/nav-left.png)' );
				gdl_print_style( "div.gdl-twitter-navigation a.prev", $temp_att );		
			$temp_att = gdl_style_att( 'background-image', 'url(' . GOODLAYERS_PATH . '/images/icon/' . $gdl_twitter_icon_type . '/nav-right.png)' );
				gdl_print_style( "div.gdl-twitter-navigation a.next", $temp_att );						
				
			// Retina
			gdl_write_data( '@media only screen and (min--moz-device-pixel-ratio: 2), only screen and (-o-min-device-pixel-ratio: 2/1),' );
			gdl_write_data( 'only screen and (-webkit-min-device-pixel-ratio: 2), only screen and (min-device-pixel-ratio: 2) {' );
			$temp_att = gdl_style_att( 'background-image', 'url(' . GOODLAYERS_PATH . '/images/icon/' . $gdl_footer_icon_type . '/nav-left@2x.png)' );
			gdl_print_style( 'div.gdl-twitter-navigation a.prev', $temp_att );				
			$temp_att = gdl_style_att( 'background-image', 'url(' . GOODLAYERS_PATH . '/images/icon/' . $gdl_footer_icon_type . '/nav-right@2x.png)' );
			gdl_print_style( 'div.gdl-twitter-navigation a.next', $temp_att );				
			gdl_write_data( '}' );				
		...
		$temp_val2 = gdl_hex_darker( $temp_val, 20 );
		...
		if($used_font != 'default -'){
			if($all_font[$used_font]['type'] == 'Cufon'){
				//gdl_write_data('div.main-navigation-wrapper{ visibility: hidden; }');
			}
		}		
